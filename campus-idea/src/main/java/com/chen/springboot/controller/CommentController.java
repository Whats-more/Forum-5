...
public class CommentController {
    ...
    @GetMapping("/tree/{articlesId}")
    public Result findTree(@PathVariable Integer articlesId) {
        List<Comment> articlesComments = commentService.findCommentDetail(articlesId);  // 查询所有的评论和回复数据
        // 查询评论数据（不包括回复）
        List<Comment> originList = articlesComments.stream().filter(comment -> comment.getOriginId() == null).collect(Collectors.toList());

        // 设置评论数据的子节点，也就是回复内容
        for (Comment origin : originList) {
            List<Comment> comments = articlesComments.stream().filter(comment -> origin.getId().equals(comment.getOriginId())).collect(Collectors.toList());  // 表示回复对象集合
            comments.forEach(comment -> {
                Optional<Comment> pComment = articlesComments.stream().filter(c1 -> c1.getId().equals(comment.getPid())).findFirst();  // 找到当前评论的父级
    ...
    public Result findPage(@RequestParam (defaultValue = "") String content,
        ...
        if (StrUtil.isNotBlank(content)) {         //查询
