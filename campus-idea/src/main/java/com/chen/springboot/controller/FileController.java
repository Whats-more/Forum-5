...
import com.chen.springboot.mapper.FileMapper;
...
public class FileController {
    ...
    public String upload(@RequestParam MultipartFile file) throws IOException {
        String originalFilename = file.getOriginalFilename();  //获取文件名字
        String type = FileUtil.extName(originalFilename);      //获取文件类型
        long size = file.getSize();                            //获取文件大小

        // 定义一个文件唯一的标识码   完整的文件存储路径就是fileUploadPath + fileUUID
        ...
        String url;
        // 获取文件的md5        比对文件的md5是否相同，相同的话直接使用，就再重复存储了
...
//    @CachePut(value = "files", key = "'frontAll'")       springboot的缓存
    @PostMapping("/update")  //更新的表中的状态
    ...
    @GetMapping("/detail/{id}")          //逻辑删除   数据库里面改成1就回来了
